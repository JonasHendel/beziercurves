...
const QuadraticBezier = ({ controlPoints, t, findPointsBetween, handlePointer }) => {
	...
	return (
		<svg xmlns='http://www.w3.org/2000/svg' width='full' height='full' viewBox='0 0 841.9 595.3'>
			<path
				d={`M ${controlPoints[0].x} ${controlPoints[0].y}  q ${controlPoints[1].x - controlPoints[0].x} ${controlPoints[1].y - controlPoints[0].y} ${controlPoints[2].x - controlPoints[0].x} ${
			...
			{intermediatePoints.length > 0 && (
				<path d={`M${intermediatePoints[0].x} ${intermediatePoints[0].y} L${intermediatePoints[1].x} ${intermediatePoints[1].y}`} stroke='#e31e63' strokeWidth='2' className='z-10' />
			)}
			<circle
